...
// UDP definitions by Sasha
#define UDP_SRC_PORT        IP_DATA_OFS + 0
#define UDP_DST_PORT        IP_DATA_OFS + 2
#define UDP_LEN_DGRM        IP_DATA_OFS + 4
#define UDP_CKSUM           IP_DATA_OFS + 6
#define UDP_DATA            IP_DATA_OFS + 8
...
typedef struct {
    ...
    uint16_t Flags_Offset;
    uint16_t ProtocolType_TTL;
    uint16_t SRC_IP_Frame;          // контрольная сумма заголовка
    uint16_t SenderIPAddr[2];
    uint16_t TargetIPAddr[2];
    // UDP
    uint16_t SRC_port;              // параметры
    uint16_t DEST_port;
    uint16_t len;
    uint16_t cksum;
    uint16_t data[];

} sEthernetUDP_Frame, *pEthernetUDP_Frame;
